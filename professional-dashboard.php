                    ...
                    <div class="payment-list">
                        ...
                        <?php else: ?>
                            <?php foreach ($payments as $payment): ?>
                                <div class="payment-card" data-date="<?= date('Y-m-d', strtotime($payment['date'])) ?>">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('paymentFilter').addEventListener('change', function() {
            ...
            paymentCards.forEach(card => {
                const paymentDateStr = card.getAttribute('data-date');
                ...
                switch(filterValue) {
                    ...
                    case 'This Month':
                        shouldShow = paymentDate.getMonth() === today.getMonth() && 
                    ...
                    case 'Last Month':
                        ...
                        shouldShow = paymentDate.getMonth() === lastMonth && 
            ...
            // Update payment stats for visible cards
            updatePaymentStats();
        });

        // Function to update payment stats based on visible cards
        function updatePaymentStats() {
            const visiblePayments = document.querySelectorAll('.payment-card');
            let thisMonthTotal = 0;
            let allTimeTotal = 0;
            const today = new Date();
            
            visiblePayments.forEach(card => {
                if (card.style.display !== 'none') {
                    const paymentDateStr = card.getAttribute('data-date');
                    const paymentDate = new Date(paymentDateStr);
                    const amountText = card.querySelector('.payment-amount h4').textContent;
            ...
            document.querySelector('.payment-stat:nth-child(1) h4').textContent = '₹' + thisMonthTotal.toFixed(2);
            document.querySelector('.payment-stat:nth-child(2) h4').textContent = '₹' + allTimeTotal.toFixed(2);
        }
